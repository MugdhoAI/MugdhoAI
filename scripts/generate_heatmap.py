...
def render(output, static=False):
    ...
    parts.append(f'<text x="28" y="58" fill="{MUTED}" font-size="10">$ neofetch --profile MugdhoAI</text>')
    parts.append(f'<text x="28" y="92" fill="{INK}" font-size="18" font-weight="700">MUGDHOAI</text>')
    parts.append(f'<text x="28" y="111" fill="{MUTED}" font-size="9">AI ENGINEERING · PRODUCTS · OPEN SOURCE</text>')
    start_y=154; step=46
    for i,(k,v) in enumerate(ROWS):
        ...
        parts.append(f'<g opacity="{opacity}"><text x="30" y="{y}" fill="{MUTED}" font-size="9">{esc(k.upper())}</text><text x="132" y="{y}" fill="{INK}" font-size="11">{esc(v)}</text>{anim}</g>')
    y=445
    ...
    for i,v in enumerate(focus):
        ...
        opacity='1' if static else '0'
        anim='' if static else f'<animate attributeName="opacity" from="0" to="1" begin="{delay:.2f}s" dur="0.30s" fill="freeze"/>'
        parts.append(f'<g opacity="{opacity}"><text x="30" y="{yy}" fill="{GREEN_4}" font-size="9">›</text><text x="45" y="{yy}" fill="{INK}" font-size="10">{esc(v)}</text>{anim}</g>')
    parts.append(f'<text x="28" y="{H-20}" fill="{MUTED}" font-size="9">github.com/MugdhoAI <tspan fill="{GREEN_4}">●</tspan> building in public</text>')
